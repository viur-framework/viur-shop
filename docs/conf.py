...
_version_spec = importlib.util.spec_from_file_location(
    "version", f"../src/viur/shop/version.py")
...
htmlhelp_basename = "viur-shop-doc"


def skip_autoapi_member(app, what, name, obj, skip, options):
    """Skip private attributes"""
    # print(f"[DEBUG] {what=} | {name=} | {vars(obj)=} | {options=}")
    if what in {"attribute", "data"} and obj.name.startswith("_"):
        print(f"Skipping <{what}>{name}")
        return True
    return skip


def setup(app):
    app.connect("autoapi-skip-member", skip_autoapi_member)
